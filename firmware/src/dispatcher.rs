...
impl AirtimeBudget {
    pub const fn new() -> Self {
        Self {
            records: [TxRecord { start_ms: 0, duration_ms: 0 }; BUDGET_SLOTS],
            head: 0,
            count: 0,
        }
    }
    ...
    pub fn record_tx(&mut self, now_ms: u64, duration_ms: u32) {
        self.records[self.head] = TxRecord { start_ms: now_ms, duration_ms };
...
mod tests {
    ...
    fn dedup_ring_evicts_oldest() {
        ...
        let mut frames: Vec<Vec<u8>> = (0..DEDUP_SLOTS)
            .map(|i| vec![i as u8; 8])
            .collect();
        for fr in &frames {
            f.insert(fr);
        ...
        assert!(!f.is_duplicate(&frames[0]), "oldest should have been evicted");
    }
    ...
    fn dedup_relayed_copy_with_mutated_path_is_duplicate() {
        ...
        f.insert(&direct);
        assert!(f.is_duplicate(&relayed), "relayed copy must dedup against original");
    }
    ...
    fn budget_enforces_limit() {
        ...
        assert!(b.can_transmit(100, 99), "should be allowed: just within budget");
    }
    ...
    fn txqueue_both_frames_enqueued_same_pass_survive_fifo_order() {
        ...
        assert_eq!(&buf[..n1], b"first", "oldest frame must drain first, not be dropped");
        q.pop_front();
    ...
    fn txqueue_peek_does_not_consume_frame() {
        ...
        assert!(q.has_pending(), "peek must not remove the frame from the queue");
    ...
    fn airtime_max_frame_under_500ms() {
        ...
        assert!(ms < 1000, "max frame airtime {} ms exceeds 1000 ms budget", ms);
    }
}
