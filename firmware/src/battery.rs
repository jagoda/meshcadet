...
impl BatteryStatus {
    ...
    pub const fn unknown() -> Self {
        BatteryStatus { percent: 0, charging: false, raw_mv: 0, held_raw_mv: 0 }
    }
}
...
mod tests {
    ...
    fn percent_full_rested_pack_reads_at_or_near_100() {
        ...
        for mv in [4_100u32, 4_120, 4_150] {
            ...
            assert!(p >= 90, "expected >=90% at {mv}mV (rested-full range), got {p}%");
        }
    }
    ...
    fn reading_one_mv_over_threshold_is_already_impossible() {
        ...
        let (settled, charging) = battery_poll_step(settled_before, EXTERNAL_POWER_MV_THRESHOLD + 1);
        assert!(charging, "one mV over the ceiling must already be treated as external power present");
        assert_eq!(settled, settled_before, "basis must freeze, not adopt the just-over-ceiling reading");
    }
    ...
    fn reading_above_threshold_holds_last_basis_and_reports_power() {
        ...
        assert!(charging, "an impossible-for-a-battery reading means external power is present");
        assert_eq!(
    ...
    fn engages_on_the_very_first_poll_with_no_prior_history() {
        ...
        assert!(charging, "must detect power-present on the very first sample, no prior baseline required");
    }
    ...
    fn boot_already_on_power_is_flagged_charging_immediately() {
        ...
        assert!(charging, "boot-on-power must be detected on the initial sample");
    ...
    fn holds_basis_indefinitely_while_above_threshold_no_plateau_confusion() {
        ...
        for _ in 0..10 {
            ...
            assert!(charging, "must not drop to 'not charging' while still above the ceiling");
            assert_eq!(settled, settled_before, "basis must stay frozen the whole time on power");
        }
    }
    ...
    fn drop_back_under_threshold_ends_power_and_resyncs_basis() {
        ...
        assert!(!charging, "a reading back under the ceiling means external power is gone");
        assert_eq!(
            settled, 3_900,
    ...
    fn full_plug_unplug_cycle_never_reports_a_false_100_percent() {
        ...
        for _ in 0..10 {
            (settled, charging) = battery_poll_step(settled, 4888);
            assert!(charging, "must not drop to 'not charging' while still on power");
            assert_eq!(percent_from_millivolts(settled), 36, "must stay at the true SoC while on power");
        }
