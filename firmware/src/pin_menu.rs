...
pub fn apply_menu_action(action: &MenuAction, settings: &mut RuntimeSettings) {
    match action {
        ...
        MenuAction::SetContactTelemetry { contact_index, enabled } => {
...
mod tests {
    ...
    fn set_contact_telemetry_enable() {
        let mut s = RuntimeSettings::default();
        apply_menu_action(
            &MenuAction::SetContactTelemetry { contact_index: 3, enabled: true },
            &mut s,
        );
    ...
    fn set_contact_telemetry_disable() {
        ...
        apply_menu_action(
            &MenuAction::SetContactTelemetry { contact_index: 7, enabled: false },
            &mut s,
        );
    ...
    fn set_contact_telemetry_out_of_bounds_ignored() {
        ...
        apply_menu_action(
            &MenuAction::SetContactTelemetry { contact_index: MAX_CONTACTS, enabled: true },
            &mut s,
        );
    ...
    fn pin_menu_acceptance_gate() {
        ...
        assert!(!settings.notif_audible, "action must apply after PIN accepted");
    }
    ...
    fn multiple_actions_accumulate() {
        ...
        apply_menu_action(&MenuAction::SetNotifAudible(false), &mut s);
        apply_menu_action(
            &MenuAction::SetContactTelemetry { contact_index: 0, enabled: true },
            &mut s,
        );
