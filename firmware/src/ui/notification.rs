...
impl Default for NotifPref {
    fn default() -> Self {
        NotifPref { visual: true, audible: true }
    }
}
...
impl Default for NotifPrefs {
    ...
    fn default() -> Self {
        NotifPrefs {
            incoming_dm:         NotifPref { visual: true,  audible: true  },
            incoming_group_msg:  NotifPref { visual: true,  audible: true  },
            dm_acked:            NotifPref { visual: true,  audible: false },
            channel_acked:       NotifPref { visual: true,  audible: false },
            provisioned:         NotifPref { visual: true,  audible: true  },
            telemetry_response:  NotifPref { visual: true,  audible: false },
            pin_error:           NotifPref { visual: true,  audible: true  },
            pin_success:         NotifPref { visual: true,  audible: true  },
        }
    }
}
...
impl NotifPrefs {
    ...
    pub fn from_provisioning_defaults(visual: bool, audible: bool) -> Self {
        ...
        NotifPrefs {
            ...
            dm_acked:            NotifPref { visual, audible: false }, // ack never audible by default
            channel_acked:       NotifPref { visual, audible: false }, // ack never audible by default
            provisioned:         pref.clone(),
            telemetry_response:  NotifPref { visual, audible: false },
...
pub fn tone_sequence(event: NotifEvent) -> &'static [ToneBurst] {
    match event {
        NotifEvent::IncomingDm => &[
            ToneBurst { freq_hz: 880,  dur_ms: 120 },
            ToneBurst { freq_hz: 1320, dur_ms: 80  },
        ],
        NotifEvent::IncomingGroupMsg => &[
            ToneBurst { freq_hz: 660,  dur_ms: 100 },
            ToneBurst { freq_hz: 880,  dur_ms: 100 },
        ],
        NotifEvent::DmAcked => &[
            ToneBurst { freq_hz: 1047, dur_ms: 60 }, // brief soft tick
        ],
        NotifEvent::ChannelAcked => &[
            ToneBurst { freq_hz: 1047, dur_ms: 60 }, // same brief soft tick as DmAcked
        ],
        NotifEvent::Provisioned => &[
            ToneBurst { freq_hz: 523,  dur_ms: 150 },
            ToneBurst { freq_hz: 784,  dur_ms: 150 },
            ToneBurst { freq_hz: 1047, dur_ms: 250 },
        ],
        NotifEvent::TelemetryResponse => &[
            ToneBurst { freq_hz: 880,  dur_ms: 80 },
        ],
        NotifEvent::PinError => &[
            ToneBurst { freq_hz: 330,  dur_ms: 200 },
            ToneBurst { freq_hz: 220,  dur_ms: 300 },
        ],
        NotifEvent::PinSuccess => &[
            ToneBurst { freq_hz: 784,  dur_ms: 100 },
            ToneBurst { freq_hz: 1047, dur_ms: 150 },
        ],
    }
}
...
impl BlinkLoop {
    ...
    pub fn poll(&mut self, now_ms: u64) -> bool {
        ...
        phase % 2 == 0 // even phases are "on", odd phases are "off"
    }
}
...
mod tests {
    ...
    fn set_event_pref_override() {
        ...
        prefs.set_event_pref(NotifEvent::IncomingDm, NotifPref { visual: false, audible: false });
        assert!(!prefs.incoming_dm.visual);
        assert!(!prefs.incoming_dm.audible);
    }
    ...
    fn dispatcher_fire_is_audio_only_for_non_incoming_event() {
        // Non-incoming events (e.g. DmAcked) have no visual notification at
        // all now that the border flash is gone — only audible fires.
        let mut d = NotifDispatcher::new(NotifPrefs::default());
        d.fire(NotifEvent::DmAcked, 0, false);
    ...
    fn dispatcher_no_fire_when_both_off() {
        let mut d = NotifDispatcher::new(NotifPrefs {
            incoming_dm: NotifPref { visual: false, audible: false },
            ..NotifPrefs::default()
        });
        d.fire(NotifEvent::IncomingDm, 0, false);
        assert!(!d.blink_active());
    ...
    fn pin_error_tone_sequence_descends() {
        ...
        assert!(seq[0].freq_hz > seq[seq.len() - 1].freq_hz,
    ...
    fn incoming_asleep_visual_off_does_not_start_blink() {
        let mut d = NotifDispatcher::new(NotifPrefs {
            incoming_dm: NotifPref { visual: false, audible: true },
            ..NotifPrefs::default()
        });
    ...
    fn second_message_while_asleep_keeps_single_loop() {
        ...
        assert!(d.poll_blink(200)); // still mid-burst, "on"
