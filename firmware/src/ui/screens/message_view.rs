...
/// count past a previously-recorded value does. Mirrors `contact_list.rs`'s
/// `unread_total_increased` (same shape, different underlying quantity);
/// pulled out as a pure function for the same "Slint root can't be
/// constructed off-device" host-testability reason that file's doc
/// documents.
fn received_total_increased(prev: Option<i32>, total: i32) -> bool {
...
// comet-on-notify baseline/increase predicate, pulled out to a pure function
// for the same "Slint root can't be constructed off-device" reason
// `contact_list.rs`'s equivalent test module documents.
#[cfg(test)]
mod tests {
