...
/// `"9+"` above — the single clamp rule shared by every unread badge this
/// screen renders (per-row `ContactRow`/channel-row badges AND the two
/// tab-bar aggregate badges in `set_contacts`/`set_channels` below). Kept as
/// one pure, unit-testable function rather than four independently-inlined
/// copies of the same ternary, so the "9+" rule can't drift between them.
fn format_unread_badge(count: i32) -> String {
...
fn unread_total_increased(prev: Option<i32>, total: i32) -> bool {
...
// per-tab aggregate badges (`contacts_unread_total`/`channels_unread_total`
// and their `_str` counterparts, set in `set_contacts`/`set_channels` above)
// had zero direct test coverage before this — the Slint root component can't
// be constructed off-device (no display backend in a host/CI test run), so
// the arithmetic/formatting was previously only exercised by hand on
// hardware. Pulling the shared clamp rule out to `format_unread_badge` makes
// at least that seam host-testable; `UiRuntime::build_channel_items`
// (firmware/src/ui/mod.rs) already pins the upstream "does the unread map
// entry reach the per-row `unread` field" half of the pipeline.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    // Regression guard: the
    // comet-on-notify baseline/increase predicate, pulled out to a pure
    // function for the same "Slint root can't be constructed off-device"
    // reason the unread-badge tests above already document.
    #[test]
