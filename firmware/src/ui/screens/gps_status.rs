...
//!
//! All display strings are formatted Rust-side (`format_*` below) and passed
//! to Slint as plain text — the same convention used throughout this UI
//! (e.g. `admin_menu::format_screen_sleep`).
//!
...
/// (`Acquiring`) — see the module doc for why this beats a has-fix boolean.
fn format_fix_state(state: crate::gps::FixState) -> &'static str {
...
mod tests {
    ...
    fn fix_state_no_signal() {
        assert_eq!(format_fix_state(crate::gps::FixState::NoSignal), "No signal");
    }

    #[test]
    fn fix_state_acquiring() {
        assert_eq!(format_fix_state(crate::gps::FixState::Acquiring), "Acquiring\u{2026}");
    }
