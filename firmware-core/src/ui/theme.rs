...
//! RGB565 twins of the Slint `Theme` global (`firmware/src/ui/theme.slint`).
//!
...
//!
//! This whole module is plain Rust with no Slint or ESP-IDF dependency, so
//! it moves here in full (unlike its screen-adjacent siblings, there is no
//! Slint-backed wrapper left behind in `firmware/src/ui/theme.rs` — that
//! file becomes a `pub use firmware_core::ui::theme::*;` shim) so its tests
//! execute under `cargo test --workspace`. See
