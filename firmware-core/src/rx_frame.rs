...
//! # Why this exists
//!
//! MeshCore's wire framing (`src/Packet.cpp::readFrom` / `writeTo` @
//! `companion-v1.17.1`, unchanged since `dee3e26a`) is:
//!
//! ```text
...
//! (`0x03`) — `Packet::hasTransportCodes()`. `on_receive` previously computed
//! `payload_off = 2 + path_bytes` unconditionally, i.e. always assumed
//! `path_len` sits at byte offset 1 — correct for `ROUTE_TYPE_FLOOD`/`DIRECT`
//! (MeshCadet's own only-ever-emitted route types), but wrong for a received
//! transport-coded frame: `path_len` actually sits at offset 5 there, so the
//! old code read the first transport-code byte as `path_len`, producing a
//! garbage path length and misparsing (silently dropping, at best) the rest
//! of the frame.
//!
//! Splitting this out of `firmware/src/main.rs` (a detached, xtensa-only
//! Cargo workspace whose `#[cfg(test)]` blocks type-check but never execute —
//! see `docs/adr/0005-firmware-core-extraction.md`) into this host-testable
//! crate is what lets `cargo test -p firmware-core` actually exercise the
//! transport-code branch and the `PathLen::is_valid()` early-reject.

use protocol::{Header, PathLen, RouteType};
...
mod tests {
    use super::*;
    use protocol::{Header, PayloadType};
