...
use protocol::{
    Header, PathLen, PayloadType, RouteType,
    Identity,
    compute_ack_hash, decode_dm_payload, decode_grp_txt_var, decode_txt_msg_plaintext,
    encode_dm_payload, encode_grp_txt_var, encode_txt_msg_plaintext,
};
...
fn parse_frame(frame: &[u8]) -> (u8, &[u8]) {
    assert!(frame.len() >= 2, "frame must carry at least header + path_len");
    ...
    assert!(frame.len() >= payload_off, "frame shorter than encoded path");
...
fn dm_frame_on_air_contract() {
    ...
    assert_eq!(frame[0], 0x09, "flood TXT_MSG header must be 0x09 (TXT_MSG<<2 | FLOOD)");
    assert_eq!(frame[1], FLOOD_PATH_LEN, "path_len must be 0x40 (2-byte hash, 0 hops)");
    assert_eq!(frame[PAYLOAD_OFFSET], receiver.pub_hash(), "DM payload[0] = dest_hash");
    assert_eq!(frame[PAYLOAD_OFFSET + 1], sender.pub_hash(), "DM payload[1] = src_hash");

    // ── Receiver parses the composed frame and recovers everything ────────────
    let (ptype, payload) = parse_frame(&frame[..frame_len]);
    assert_eq!(ptype, PayloadType::TxtMsg as u8, "parsed payload type must be TXT_MSG");
    ...
    let end = text_region.iter().position(|&b| b == 0).unwrap_or(text_region.len());
    assert_eq!(rx_ts, timestamp, "timestamp survives the full frame round-trip");
    assert_eq!(rx_type, 0, "txt_type survives");
    assert_eq!(&text_region[..end], text, "text survives the full frame round-trip");
}
...
fn grp_txt_frame_on_air_contract() {
    ...
    frame[1] = PathLen::new(2, 0).unwrap().0;
    let n = encode_grp_txt_var(&channel_secret, timestamp, 0, 0, text, &mut frame[PAYLOAD_OFFSET..]);
    let frame_len = PAYLOAD_OFFSET + n;

    assert_eq!(frame[0], 0x15, "flood GRP_TXT header must be 0x15 (GRP_TXT<<2 | FLOOD)");
    ...
    let (ptype, payload) = parse_frame(&frame[..frame_len]);
    assert_eq!(ptype, PayloadType::GrpTxt as u8, "parsed payload type must be GRP_TXT");

    let mut pt = [0u8; 256];
    let fields = decode_grp_txt_var(&channel_secret, payload, &mut pt).unwrap();
    assert_eq!(fields.timestamp, timestamp, "timestamp survives the full frame round-trip");
    let rx_text = &pt[fields.text_offset..fields.text_offset + fields.text_len];
    let end = rx_text.iter().position(|&b| b == 0).unwrap_or(rx_text.len());
    assert_eq!(&rx_text[..end], text, "text survives the full frame round-trip");
}
...
fn ack_frame_on_air_contract() {
    ...
    assert_eq!(frame[0], 0x0D, "flood ACK header must be 0x0D (ACK<<2 | FLOOD)");
    assert_eq!(frame[1], FLOOD_PATH_LEN, "path_len must be 0x40");

    let (ptype, payload) = parse_frame(&frame[..frame_len]);
    assert_eq!(ptype, PayloadType::Ack as u8, "parsed payload type must be ACK");
    assert_eq!(payload.len(), 4, "v1.15 ACK payload is exactly 4 bytes");
    assert_eq!(payload, &ack_hash, "ACK hash recovered intact from the composed frame");
}
...
fn flood_path_len_decodes_to_zero_path() {
    ...
    assert_eq!(pl.path_byte_len(), 0, "0 path bytes ⇒ payload sits at offset 2");
}
