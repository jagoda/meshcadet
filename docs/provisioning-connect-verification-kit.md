...
- Click **Connect** and select the T-Deck Plus's port in the browser's
  device picker.
- **Expect:** the page shows device status (pubkey, "0 contacts, 0
  channels") within a couple of seconds. The serial monitor shows normal
  `prov_server: QUERY_STATUS` log lines — **no panic, no reboot banner, no
  gap where the monitor goes silent and a fresh boot banner appears.**
- **If the device reboots:** capture, verbatim, into the result block below:
  - The full serial monitor output from immediately before the click through
    the reboot (a `panic`/`Guru Meditation Error` line here is the single
    most useful new fact this kit can produce — it names the exact line that
    crashed).
  - Whether the monitor shows a firmware panic at all, or instead goes
    ...
    panic text in between (this distinguishes a firmware-side crash from a
    hardware-level reset — e.g. the browser's Web Serial implementation
    asserting DTR/RTS on `port.open()`, which a source-only read could not
    check — see ADR-0002's transport notes on why the host
    CLI deliberately leaves DTR/RTS untouched).
