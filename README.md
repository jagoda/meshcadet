...
SECRET=$(cargo run -q -p host -- gen-channel-secret --bits 256)
cargo run -p host -- --port /dev/ttyACM0 add-channel --secret "$SECRET" --name "family" --primary
...
hex string too (e.g. one shared out-of-band by another admin), but
`gen-channel-secret` is how you produce one with a real entropy floor;
`add-channel` warns on stderr if the supplied secret looks like an obviously
weak placeholder (all-zero, a repeated byte, a simple sequential run).
