...
use crate::transport::{SendTimedOut, Transport};
...
// ── Connect-triggered reset recovery ──────────────────────────────────────────

/// How long to wait after a `SendTimedOut` before reopening the port and
/// retrying the command once.
///
/// CONFIRMED BY KERNEL EVIDENCE (round 7, `meshcadet-connect-wedge-round7-
/// stale-handle-reenumeration`, 2026-09-22): opening the port asserts
/// DTR/RTS, which resets the ESP32-S3's USB-Serial-JTAG chip and the chip
/// then re-enumerates on USB. The device-confirmed boot timeline shows the
/// application reaching "admin server thread started" at 2869ms after reset.
/// 3.5s is comfortably past that (≈630ms margin) while still failing fast
/// relative to a human waiting at a terminal — the USB re-enumeration itself
/// (the kernel's "New USB device found" / fresh `cdc_acm` attach) completes
/// well before the application finishes booting, so this margin covers both.
pub const RESET_SETTLE_DELAY: Duration = Duration::from_millis(3500);

/// Run `cmd` once against `session`. If it fails with `SendTimedOut` —
/// round 7's confirmed connect-triggered device reset, which leaves
/// `session`'s transport holding a stale handle into a USB interface the
/// kernel has already torn down — wait `settle` for the device to finish
/// re-enumerating, build a fresh transport via `reopen`, and retry `cmd`
/// exactly once more against a brand-new `Session` wrapping it. Any other
/// error (including a second `SendTimedOut` on the retry — this recovers
/// from the one confirmed failure mode, not an unbounded retry loop)
/// propagates unchanged.
///
/// Generic over `T: Transport` rather than hardcoded to `SerialTransport` so
/// the recovery DECISION is unit-testable without a real serial port (see
/// the tests below); `host/src/main.rs` is the real caller, with `reopen` =
/// `|| SerialTransport::open(port, baud)` and `settle` = `RESET_SETTLE_DELAY`.
pub fn run_with_reset_recovery<T, F>(
    mut session: Session<T>,
    reopen: impl FnOnce() -> anyhow::Result<T>,
    settle: Duration,
    mut cmd: F,
) -> anyhow::Result<()>
where
    T: Transport,
    F: FnMut(&mut Session<T>) -> anyhow::Result<()>,
{
    match cmd(&mut session) {
        Ok(()) => Ok(()),
        Err(e) if e.downcast_ref::<SendTimedOut>().is_some() => {
            eprintln!(
                "host CLI: {e} — this matches the confirmed connect-triggered device reset \
                 (opening the port asserts DTR/RTS, which resets the ESP32-S3 and invalidates \
                 the handle just acquired; see docs/provisioning-connect-verification-kit.md). \
                 Waiting {:?} for the device to finish re-enumerating, then reopening and \
                 retrying the command once.",
                settle
            );
            std::thread::sleep(settle);
            let transport = reopen()?;
...
#[cfg(test)]
mod reset_recovery_tests {
    use super::*;
    use std::sync::atomic::{AtomicUsize, Ordering};
    use std::sync::Arc;

    /// A `Transport` that never actually does any I/O — the tests below drive
    /// the recovery DECISION via `cmd`'s own return values, not via any real
    /// send/recv traffic, so this only needs to satisfy the trait.
    ...
    #[test]
    fn recovers_by_reopening_and_retrying_once_on_send_timed_out() {
        let session = Session::new(NoopTransport);
        let attempts = Arc::new(AtomicUsize::new(0));
        let reopen_calls = Arc::new(AtomicUsize::new(0));

        let attempts_in_cmd = Arc::clone(&attempts);
        let reopen_calls_in_reopen = Arc::clone(&reopen_calls);

        let result = run_with_reset_recovery(
            session,
            move || {
                reopen_calls_in_reopen.fetch_add(1, Ordering::SeqCst);
                Ok(NoopTransport)
            },
            Duration::from_millis(1),
            move |_session: &mut Session<NoopTransport>| {
                if attempts_in_cmd.fetch_add(1, Ordering::SeqCst) == 0 {
                    Err(anyhow::Error::new(SendTimedOut {
        ...
        assert_eq!(
            ...
            2,
            "cmd must run exactly twice: once, then once more against the reopened port"
        );
        assert_eq!(
            reopen_calls.load(Ordering::SeqCst),
            1,
            "reopen must be called exactly once"
        );
    }

    #[test]
    fn does_not_reopen_or_retry_a_non_reset_error() {
        let session = Session::new(NoopTransport);
        let reopen_calls = Arc::new(AtomicUsize::new(0));
        let reopen_calls_in_reopen = Arc::clone(&reopen_calls);
        ...
        let result = run_with_reset_recovery(
            ...
            Duration::from_millis(1),
            |_session: &mut Session<NoopTransport>| anyhow::bail!("some unrelated device error"),
        );
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("some unrelated device error"));
        assert_eq!(
            reopen_calls.load(Ordering::SeqCst),
            0,
            "an error other than SendTimedOut must never trigger reopen/retry"
        );
    }

    #[test]
    fn a_second_send_timed_out_on_the_retry_still_propagates() {
        // Confirms this is bounded to exactly one retry, not an unbounded
        // loop, if the device is still unreachable after the settle delay.
        let session = Session::new(NoopTransport);
        ...
        let result = run_with_reset_recovery(
            ...
            || Ok(NoopTransport),
            Duration::from_millis(1),
            |_session: &mut Session<NoopTransport>| {
                Err(anyhow::Error::new(SendTimedOut {
                    timeout: Duration::from_secs(3),
                }))
            },
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().downcast_ref::<SendTimedOut>().is_some());
    }
}
